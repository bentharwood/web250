...
let dogs = ['snoopy', 'scooby-doo', 'goofy', 'pluto'];
...
function deleteItem(toDelete, array) {
  const deleteIndex = array.findIndex(item => item === toDelete);
  const newArray = [
...
let kennelNoToys = deleteItem('toys', kennelItems);
...
let toolProps = Object.keys(tool);
...
let toolVals = Object.values(tool);
...
let toolEntries = Object.entries(tool);
...
let woodshop = shopEquipment.join(', ');

console.group('Task 15');
console.table(woodshop);
console.groupEnd();
...
let species = wood.split(',')
let speciesTrimmed = species.map(item => item.trim());
...
let result = mapleVariants.includes('wormy maple');
...
let curls = figuredWoods.filter(wood => wood.includes('curly'));
...
let curly = woodVariants.find(wood => wood.includes('curly'));
...
let noMaple = woodVariants.filter(wood => !wood.includes('maple'));
...
let salesAsc = _.orderBy(sales, [], ['asc']);
...
let teamDesc = _.orderBy(salesTeam, [],['desc']);
